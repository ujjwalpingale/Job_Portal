...
def view_profile(current_user: dict = Depends(allow_candidate)):
    """View candidate profile."""
    return current_user

@router.put("/profile")
def update_profile(profile_data: CandidateProfileUpdate, current_user: dict = Depends(allow_candidate)):
...
def upload_resume(file: UploadFile = File(...), current_user: dict = Depends(allow_candidate)):
    ...
    updated_user = update_user_profile(current_user["id"], {"resume_url": file_path})
    
    return success_response(message="Resume uploaded successfully", data={"resume_url": file_path})

@router.get("/applications")
def view_applied_jobs(current_user: dict = Depends(allow_candidate)):
